...
get_partitions() {
    ...
    if [ $? -ne 0 ]; then
        echo "Błąd połączenia SSH z hostem $hostname"
        return 1
    fi
    ...
    echo "$partitions" | while read -r line; do
        ...
        if [[ $filesystem =~ ^/dev/mapper ]]; then
            ...
            if [ "$existing_partition" -eq 0 ]; then
                echo "Znaleziono nową partycję: $filesystem" 
                server_partition=$filesystem  

                echo "Dodaję partycję $server_partition do bazy danych dla hosta $hostname"
                ...
                "
                echo "Dodano partycję ($server_partition) do bazy danych."
            else
                echo "Partycja $filesystem już istnieje w bazie danych. Pomijam dodawanie."
            fi
        fi
    done   
}
...
remove_nonexistent_partitions() {
    ...
    for hostname in $host_data; do
        echo "Sprawdzam partycje na hoście $hostname..."
        ...
        for resource in "${partition_array[@]}"; do
            ...
            if [ $? -ne 0 ]; then
                echo "Partycja $mount_point ($filesystem) nie istnieje na hoście $hostname. Usuwam ją z bazy danych."
                mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASS" -D "$DB_NAME" -e "
                    DELETE FROM partitions WHERE host = '$hostname' AND mount_point = '$mount_point' AND filesystem = '$filesystem';
                "
                echo "Usunięto partycję $mount_point ($filesystem) z bazy danych."
            fi
        done
    done
}
...
IFS=$'\n'
for hostname in $host_data; do
    echo "Pobieranie danych z hosta $hostname..."
