...
get_pv_info() {
    ...
    if [ $? -ne 0 ]; then
        echo "Error connecting to host $host. Skipping."
        return
    fi
...
for host in "${HOSTS[@]}"; do
    echo "Fetching data from host $host..."
    ...
    while read -r filesystem mount_point total_space used_space available_space usage_percent; do
        ...
        if [[ "$filesystem" == /dev/mapper* ]] && [ "$usage_percent" -gt "$THRESHOLD" ]; then
            MESSAGE="Alert: Disk space on host $host, filesystem $filesystem ($mount_point) has exceeded the threshold $THRESHOLD% ($usage_percent%)"
    ...
    for resource in "${disk_resources[@]}"; do
        ...
        if [ $? -ne 0 ]; then
            echo "Resource $mount_point ($filesystem) does not exist on server $host. Removing it from the database."
            mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" -e \
            "DELETE FROM disk_usage WHERE hostname='$host' AND mount_point='$mount_point' AND filesystem='$filesystem';"
            if [ $? -eq 0 ]; then
                echo "Removed non-existent resource: $host, $mount_point, $filesystem"
            else
                echo "Error removing resource: $host, $mount_point, $filesystem"
            fi
        fi
    done
done
